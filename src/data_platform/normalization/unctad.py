...
def normalize_unctad_snapshot(
    ...
    *,
    ...
    for column in ["shipping_lsci_index", "shipping_port_connectivity_index"]:
        normalized[column] = pd.to_numeric(normalized[column], errors="coerce")
    publication_value = (
        publication_ts_utc if publication_ts_utc is not None and not pd.isna(publication_ts_utc) else normalized["observation_date"].max()
    )
    ...
    normalized["source_name"] = "unctad"
    normalized["source_record_id"] = (
        normalized["country_iso3"].astype("string") + "-" + normalized["observation_date"].dt.strftime("%Y-%m-%d")
    )
    normalized = normalized.loc[normalized["country_iso3"].notna() & normalized["observation_date"].notna()].copy()
    return normalized[
        [
            "source_name",
            "source_record_id",
    ...
    ].sort_values(by=["country_iso3", "observation_date"])
